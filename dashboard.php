<div class="main">
    <div class="container">
        <div class="row">
            <div class="span6">	
                <div class="widget stacked">
                    <div class="widget-header">
                        <i class="icon-star"></i>
                        <h3>Rating Layanan Antar Jemput</h3>
                    </div> <!-- /widget-header -->

                    <div class="widget-content">

                        <div class="stats">

                            <div class="stat">
                                <span class="stat-value">12,386</span>									
                                Rute Penjemputan
                            </div> <!-- /stat -->

                            <div class="stat">
                                <span class="stat-value">9,249</span>									
                                Jumlah Supir Saat Ini 
                            </div> <!-- /stat -->

                            <div class="stat">
                                <span class="stat-value">70%</span>									
                                Profit Paguyuban
                            </div> <!-- /stat -->

                        </div> <!-- /stats -->


                        <div id="chart-stats" class="stats">

                            <div class="stat stat-chart">							
                                <div id="donut-chart" class="chart-holder"></div> <!-- #donut -->							
                            </div> <!-- /substat -->

                            <div class="stat stat-time">									
                                <span class="stat-value">12,501</span>
                                Jumlah Siswa Terdaftar
                            </div> <!-- /substat -->

                        </div> <!-- /substats -->

                    </div> <!-- /widget-content -->

                </div> <!-- /widget -->	
            </div>
            <div class="span6">	
                <div class="widget stacked">
                    <div class="widget-header">
                        <i class="icon-bookmark"></i>
                        <h3>Quick Shortcuts</h3>
                    </div> <!-- /widget-header -->
                    <div class="widget-content">
                        <div class="shortcuts">
                            <a href="index.php?pgy=siswa&page=view" class="shortcut">
                                <i class="shortcut-icon icon-list-alt"></i>
                                <span class="shortcut-label">Data Siswa</span>
                            </a>
                            <a href="index.php?pgy=supir&page=view" class="shortcut">
                                <i class="shortcut-icon icon-list-alt"></i>
                                <span class="shortcut-label">Data Supir</span>								
                            </a>
                            <a href="index.php?pgy=karyawan&page=view" class="shortcut">
                                <i class="shortcut-icon icon-list-alt"></i>
                                <span class="shortcut-label">Data Karyawan</span>								
                            </a>
                            <a href="index.php?pgy=rute&page=view" class="shortcut">
                                <i class="shortcut-icon icon-list-alt"></i>
                                <span class="shortcut-label">Data Rute</span>								
                            </a>
                            <a href="index.php?pgy=transaksi&page=view" class="shortcut">
                                <i class="shortcut-icon icon-money"></i>
                                <span class="shortcut-label">Pembayaran</span>								
                            </a>
                            <a href="index.php?pgy=laporan-laba&page=view" class="shortcut">
                                <i class="shortcut-icon icon-signal"></i>
                                <span class="shortcut-label ">Laba</span>	
                            </a>
<!--                            <a href="index.php?pgy=supir&page=view" class="shortcut">
                                <i class="shortcut-icon icon-comment"></i>
                                <span class="shortcut-label">Comments</span>								
                            </a>-->
                            <a href="index.php?pgy=user-manage&page=view" class="shortcut">
                                <i class="shortcut-icon icon-user"></i>
                                <span class="shortcut-label">Users</span>
                            </a>
<!--                            <a href="index.php?pgy=supir&page=view" class="shortcut">
                                <i class="shortcut-icon icon-file"></i>
                                <span class="shortcut-label">Notes</span>	
                            </a>
                            <a href="index.php?pgy=supir&page=view" class="shortcut">
                                <i class="shortcut-icon icon-picture"></i>
                                <span class="shortcut-label">Photos</span>	
                            </a>
                            <a href="index.php?pgy=supir&page=view" class="shortcut">
                                <i class="shortcut-icon icon-tag"></i>
                                <span class="shortcut-label">Tags</span>
                            </a>				-->
                        </div> <!-- /shortcuts -->	
                    </div> <!-- /widget-content -->
                </div> <!-- /widget -->
            </div> <!-- /span6 -->
            <div class="span12">	
                <div class="widget stacked">
                    <div class="widget-header">
                        <i class="icon-signal"></i>
                        <h3>Grafik Profit Paguyuban</h3>
                    </div> <!-- /widget-header -->
                    <div class="widget-content">					
                        <div id="area-chart" class="chart-holder"></div>					
                    </div> <!-- /widget-content -->
                </div> <!-- /widget -->
            </div>
        </div> <!-- /row -->

    </div> <!-- /container -->

</div> <!-- /main -->

